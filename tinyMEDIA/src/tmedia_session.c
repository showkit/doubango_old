...
tmedia_codecs_L_t* tmedia_session_match_codec(tmedia_session_t* self, const tsdp_header_M_t* M)
{
	...
	char *rtpmap = tsk_null, *fmtp = tsk_null, *image_attr = tsk_null, *name = tsk_null, *framerate = tsk_null;
...
int tmedia_session_mgr_set_ice_ctx(tmedia_session_mgr_t* self, struct tnet_ice_ctx_s* ctx_audio, struct tnet_ice_ctx_s* ctx_video, struct tnet_ice_ctx_s* ctx_data)
{
	if(!self){
		TSK_DEBUG_ERROR("Invalid parameter");
		return -1;
