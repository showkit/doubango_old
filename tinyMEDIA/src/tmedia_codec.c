...
int tmedia_codec_to_sdp(const tmedia_codecs_L_t* codecs, tsdp_header_M_t* m)
{
	...
	tsk_list_foreach(item, codecs){
		...
		if(is_audio || is_video || is_data){
            ...
            /* add 'framerate' attributes */
            if((framerate = tmedia_codec_sdp_att_get(codec, "framerate")))
            {
                tsk_sprintf(&temp, "%s %s", neg_format, framerate);
                tsdp_header_M_add_headers(m, TSDP_HEADER_A_VA_ARGS("framerate", temp), tsk_null) ;
                TSK_FREE(temp);
                TSK_FREE(framerate);
            }
			/* add fmtp attributes */
