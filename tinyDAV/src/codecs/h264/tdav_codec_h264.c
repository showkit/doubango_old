...
#include "showkit/codec/h264/h264.h"

#include <libavcodec/avcodec.h>

#define MIN(x,y) (x > y ? y : x)

struct h264cb_s h264_callback_s;
...
static buf_l  buffer_list ;
static size_t buffer_list_depth ;

#define TARGET_BUFFER_DEPTH                 10
...
{
	//h264_t* h264 = (h264_t*)&tdav_codec_h264_base_plugin_def_t;
	if(!self->opened){
		...
		if(tsk_striequals(param->key, "action")){
			...
			switch(action){
				case tmedia_codec_action_encode_idr:
					{
                        if(h264_callback_s.action_encode_idr_cb)
                            h264_callback_s.action_encode_idr_cb(h264_callback_s.cb_data);
						//h264->encoder.force_idr = tsk_true;
						break;
					}
				case tmedia_codec_action_bw_down:
					{
                        if(h264_callback_s.action_bw_down_cb)
                            h264_callback_s.action_bw_down_cb(h264_callback_s.cb_data);
				...
				case tmedia_codec_action_bw_up:
					{
                        if(h264_callback_s.action_bw_up_cb)
                            h264_callback_s.action_bw_up_cb(h264_callback_s.cb_data);
						//h264->encoder.quality = TSK_CLAMP(1, (h264->encoder.quality - 1), 31);
						//h264->encoder.context->global_quality = FF_QP2LAMBDA * h264->encoder.quality;
						break;
					}
...
static tsk_size_t tdav_codec_h264_decode(tmedia_codec_t* self, const void* in_data, tsk_size_t in_size, void** out_data, tsk_size_t* out_max_size, const tsk_object_t* proto_hdr)
{
   ...
   /* {
        buf_l *next = malloc(sizeof(struct buf_l_s)) ;
        
        next->nal_data = malloc(sizeof(in_size));
        
        memcpy(next->nal_data, in_data, in_size);
        next->len = in_size ;
        next->next = NULL;

        buf_l *tail = &buffer_list;
    ...
    if(buffer_list_depth < TARGET_BUFFER_DEPTH)
    {
        TSK_DEBUG_INFO("Buffering... %zu", buffer_list_depth);
        return 0 ;
    } else {
        in_data = buffer_list.next->nal_data;
        in_size = buffer_list.next->len;
	...
	*/
    ...
    if((((uint8_t*)in_data)[0] & 0x1F) == 7)
    {
        sps = sps_parse((void*)in_data, in_size);

        if(sps.aspect_ratio_info_present_flag)
        {
            h264_callback_s.aspect = (float)sps.sar_width / (float)sps.sar_height;
//            printf("aspect: %lf\n", h264_callback_s.aspect);
        }
    }
	/* get payload */
...
static tsk_bool_t tdav_codec_h264_sdp_att_match(const tmedia_codec_t* codec, const char* att_name, const char* att_value)
{
	...
	if(tsk_striequals(att_name, "fmtp")){
        ...
        unsigned int w , h , maxbr , fps;

        int found = tmedia_codec_parse_fmtp(att_value, &maxbr, &fps, &w, &h);
		...
		if(TDAV_CODEC_H264_COMMON(codec)->profile != profile){
         //   printf("profile %d doesn't match %d\n", TDAV_CODEC_H264_COMMON(codec)->profile, profile);
			return tsk_false;
		}
		else{
			...
			if(TDAV_CODEC_H264_COMMON(codec)->level != level){
				...
				TDAV_CODEC_H264_COMMON(codec)->level = level;//TSK_MIN(TDAV_CODEC_H264_COMMON(codec)->level, level);
 
                extern int gSHKBandwidthEstimateUP ;
                extern int gSHKmaxbr ;
                int lmaxbr = gSHKBandwidthEstimateUP ;

             //   TMEDIA_CODEC_VIDEO(codec)->in.fps = TMEDIA_CODEC_VIDEO(codec)->out.fps = fps;
                gSHKmaxbr = TMEDIA_CODEC_VIDEO(codec)->in.max_br = TMEDIA_CODEC_VIDEO(codec)->out.max_br = (maxbr < lmaxbr) ? maxbr : lmaxbr;
			}
		}
...
bail:
        ...
        TMEDIA_CODEC_VIDEO(codec)->in.width = MIN(TMEDIA_CODEC_VIDEO(codec)->in.width, in_width) ;
        TMEDIA_CODEC_VIDEO(codec)->in.height = MIN(TMEDIA_CODEC_VIDEO(codec)->in.height, in_height);
        TMEDIA_CODEC_VIDEO(codec)->out.width = MIN(TMEDIA_CODEC_VIDEO(codec)->out.width, in_width) ;
        TMEDIA_CODEC_VIDEO(codec)->out.height = MIN(TMEDIA_CODEC_VIDEO(codec)->out.height, in_height);

	}
    else if(tsk_striequals(att_name, "framerate"))
    {
        unsigned in_framerate;
        sscanf(att_value, "%u", &in_framerate);
        TMEDIA_CODEC_VIDEO(codec)->in.fps = MIN(TMEDIA_CODEC_VIDEO(codec)->in.fps, in_framerate);
        TMEDIA_CODEC_VIDEO(codec)->out.fps = MIN(TMEDIA_CODEC_VIDEO(codec)->out.fps, in_framerate);
...
static char* tdav_codec_h264_sdp_att_get(const tmedia_codec_t* self, const char* att_name)
{
    ...
    TSK_DEBUG_INFO("tdav_codec_h264_sdp_att_get");
	if(tsk_striequals(att_name, "fmtp")){
		char* fmtp = tsk_null;
#if 1
        extern int gSHKBandwidthEstimateDOWN ;
        int maxbr =gSHKBandwidthEstimateDOWN ;
		
		tsk_sprintf(&fmtp, "profile-level-id=%x; packetization-mode=%d; QVGA=2; MaxBR=%d ", ((TDAV_CODEC_H264_COMMON(h264)->profile << 16) | TDAV_CODEC_H264_COMMON(h264)->level), TDAV_CODEC_H264_COMMON(h264)->pack_mode, maxbr);
#else
        ...
        char * fr = tsk_null;
        tsk_sprintf(&fr, "%u", TMEDIA_CODEC_VIDEO(self)->out.fps);
        return fr;
    }
	return tsk_null;
}
